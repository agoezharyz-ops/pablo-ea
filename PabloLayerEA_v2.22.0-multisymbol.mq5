...
input group "=== AUTO-SKALA ATR (v2.22 MULTI-SYMBOL) ==="
// v2.22: supaya EA jalan di XAUUSD / BTCUSD / NAS100 / US30 tanpa ubah angka.
// Kalau ON, semua jarak (layer/pending/SL/TP/candle/trail/BEP) dihitung
// dari ATR M1 x faktor, jadi otomatis menyesuaikan skala symbol.
input bool   InpAutoScaleATR     = true;   // v2.22: aktifkan auto-skala ATR (multi-symbol)
input int    InpAutoATRPeriod    = 14;     // Periode ATR utk auto-skala
input double InpATRJarakLayer    = 0.35;   // Jarak layer   = ATR x ini (0.35 = 35% ATR)
input double InpATRPendingDist   = 0.15;   // Jarak pending = ATR x ini
input double InpATRSL            = 1.20;   // SL per posisi = ATR x ini
input double InpATRTP             = 0.55;  // TP acuan      = ATR x ini (basis skala basket)
input double InpATRTPMin          = 0.30;  // TP minimal    = ATR x ini
input double InpATRTPMax          = 2.50;  // TP maksimal   = ATR x ini
input double InpATRCandleMin      = 0.10;  // Body candle min = ATR x ini
input double InpATRTrailStart     = 0.80;  // Trail start   = ATR x ini
input double InpATRTrailStep      = 0.25;  // Trail step    = ATR x ini
input double InpATRTPSafetyCap    = 3.0;   // v2.22.3: batas TP max sbg % ATR (anti TP tak tercapai)
...
// -- v2.21.3: FILTER ARAH MA (cegah entry lawan tren) --
//   BUY hanya diizinkan kalau harga DI ATAS MA arah.
//   SELL hanya diizinkan kalau harga DI BAWAH MA arah.
input bool               InpPakaiFilterArah = true;              // v2.21.3: aktifkan filter arah MA
input ENUM_TIMEFRAMES    InpArahTF         = PERIOD_M5;          // TF MA arah (M5 = lebih stabil dari M1)
input int                InpArahMaPeriod   = 50;                 // Periode MA arah
input bool               InpArahPakaiSlope = true;               // Wajib MA miring searah (bukan datar)
...
int      hMaArah = INVALID_HANDLE;  // v2.21.3: MA utk filter arah (anti lawan-tren)
double   adxBuf[];
...
int OnInit()
  {
   ...
   if(!InpAutoScaleATR && InpJarakLayer <= 0)
     { Print("InpJarakLayer harus > 0"); return(INIT_PARAMETERS_INCORRECT); }
   if(InpEntryMode == MODE_PENDING && !InpAutoScaleATR && InpPendingDist <= 0)
   ...
   if(!InpAutoScaleATR && InpStopLoss > 0 && InpTakeProfit > 0 && InpBEPLock >= InpStopLoss)
   ...
   // v2.21.3: MA arah (filter anti lawan-tren)
   hMaArah = iMA(_Symbol, InpArahTF, InpArahMaPeriod, 0, MODE_EMA, PRICE_CLOSE);
   if(hMaArah == INVALID_HANDLE)
      Print("Peringatan: gagal buat handle MA arah, filter arah nonaktif");
   if(InpPakaiFilterArah)
      Print("Info v2.21.3: FILTER ARAH aktif - BUY hanya di atas MA", InpArahMaPeriod,
            " ", EnumToString(InpArahTF), (InpArahPakaiSlope ? " + slope naik" : ""));
...
//==================================================================
// v2.22 MULTI-SYMBOL: skala jarak dari ATR M1
//  Kalau InpAutoScaleATR=false -> pakai angka input fix (mode lama XAUUSD).
//  Kalau true -> jarak dihitung dari ATR M1 sehingga symbol apa pun
//  (XAUUSD, BTCUSD, NAS100, US30) bisa pakai EA yang sama.
//==================================================================
double ATRNow()
  {
   if(hAtr == INVALID_HANDLE) return 0.0;
   double a[];
   if(CopyBuffer(hAtr, 0, 0, 3, a) < 3) return 0.0;
   return a[0];
  }

// Skala satu nilai: fix (inpNilai) atau ATR x faktor
double Skala(double fixVal, double atrMult)
  {
   if(!InpAutoScaleATR) return fixVal;   // mode lama: angka langsung
   double atr = ATRNow();
   if(atr <= 0.0) return fixVal;         // ATR belum siap -> fallback fix
   return atr * atrMult;
  }

// Accessor jarak (dipakai di seluruh kode)
double JarakLayerNow()   { return Skala(InpJarakLayer,   InpATRJarakLayer); }
double PendingDistNow()  { return Skala(InpPendingDist,  InpATRPendingDist); }
double StopLossNow()     { return Skala(InpStopLoss,     InpATRSL); }
double TPBaseNow()       { return Skala(InpTakeProfit,   InpATRTP); }
double TrailStartNow()   { return Skala(InpTrailStart,   InpATRTrailStart); }
double TrailStepNow()    { return Skala(InpTrailStep,    InpATRTrailStep); }
...
bool GetSignals(bool &buySig, bool &sellSig)
  {
   ...
   // v2.21.3: FILTER ARAH MA (anti entry lawan-tren)
   //   BUY hanya kalau harga DI ATAS MA arah (dan MA naik kalau slope wajib).
   //   SELL hanya kalau harga DI BAWAH MA arah (dan MA turun kalau slope wajib).
   if(InpPakaiFilterArah)
     {
      if(ArahBuyOK())  { /* lolos */ } else buySig  = false;
      if(ArahSellOK()) { /* lolos */ } else sellSig = false;
     }
   return true;
  }

//==================================================================
// v2.21.3: cek arah berbasis MA.
//  ArahBuyOK()  -> true kalau boleh BUY  (harga di atas MA, MA naik)
//  ArahSellOK() -> true kalau boleh SELL (harga di bawah MA, MA turun)
//==================================================================
bool ArahBuyOK()
  {
   if(!InpPakaiFilterArah || hMaArah == INVALID_HANDLE) return true;
   double ma[4];
   if(CopyBuffer(hMaArah, 0, 0, 4, ma) < 4) return true; // data belum siap -> jangan blok
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(bid <= ma[0]) return false;                       // harga di bawah MA -> jangan BUY
   if(InpArahPakaiSlope && ma[0] <= ma[1]) return false;// MA tidak naik -> jangan BUY
   return true;
  }
...
bool ArahSellOK()
  {
   if(!InpPakaiFilterArah || hMaArah == INVALID_HANDLE) return true;
   double ma[4];
   if(CopyBuffer(hMaArah, 0, 0, 4, ma) < 4) return true;
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   if(bid >= ma[0]) return false;                       // harga di atas MA -> jangan SELL
   if(InpArahPakaiSlope && ma[0] >= ma[1]) return false;// MA tidak turun -> jangan SELL
   return true;
  }
...
double HitungTPAdaptif()
  {
   ...
   // v2.22: clamp adaptif — mode ATR pakai ATR x faktor, mode lama pakai angka fix
   double tpMin = Skala(InpTPMin, InpATRTPMin);
   double tpMax = Skala(InpTPMax, InpATRTPMax);
   if(InpAutoScaleATR)
     {
      // v2.22.3: jangan biarkan TP terlalu jauh (nanti tak pernah kena).
      double atr = ATRNow();
      if(atr > 0 && tpMax > atr * InpATRTPSafetyCap) tpMax = atr * InpATRTPSafetyCap;
     }
   tp = MathMax(tp, tpMin);
   tp = MathMin(tp, tpMax);
   return tp;
...
void LevelsFor(ENUM_ORDER_TYPE otype, double price, double &sl, double &tp)
  {
   ...
   if(StopLossNow() > 0)
     {
      double d = MathMax(StopLossNow(), minD);
...
bool OpenTrade(ENUM_ORDER_TYPE otype, int layerIdx, string tag)
  {
   ...
   double dist = MathMax(PendingDistNow(), MinStopDistance());
...
void TryEntryPending()
  {
   ...
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double minD = MathMax(PendingDistNow(), MinStopDistance());
   ...
   if(buySig && bP < InpMaxLayer)
     {
      ...
      if(!PendingExistsNear(ORDER_TYPE_BUY_LIMIT, p, MathMax(JarakLayerNow()*0.25, 10*_Point)))
   ...
   if(sellSig && sP < InpMaxLayer)
     {
      ...
      p = NormalizeDouble(p, _Digits);
      if(!PendingExistsNear(ORDER_TYPE_SELL_LIMIT, p, MathMax(JarakLayerNow()*0.25, 10*_Point)))
...
void TryLayerSide(ENUM_POSITION_TYPE side, int curCount)
  {
   ...
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double minD = MathMax(PendingDistNow(), MinStopDistance());
   double jrk  = JarakLayerNow() * gRegimeJarakMult;           // v2.22: jarak ATR x regime
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(InpGunakanBEP && (InpBEPTrigger > 0 || InpAutoScaleATR))
        {
         if(type == POSITION_TYPE_BUY)
           {
            if(bid - open >= Skala(InpBEPTrigger, InpATRTrailStart*0.67))
              {
           ...
           {
            if(open - ask >= Skala(InpBEPTrigger, InpATRTrailStart*0.67))
              {
               double sl = NormalizeDouble(open - InpBEPLock, _Digits);
      ...
      if(InpGunakanTrailing && (InpTrailStart > 0 || InpAutoScaleATR))
        {
         if(type == POSITION_TYPE_BUY)
           {
            if(bid - open >= TrailStartNow())
              {
               double sl = NormalizeDouble(bid - TrailStepNow(), _Digits);
               if(newSL == 0 || sl > newSL) newSL = sl;
              }
           }
         else
           {
            if(open - ask >= TrailStartNow())
              {
               double sl = NormalizeDouble(ask + TrailStepNow(), _Digits);
               if(newSL == 0 || sl < newSL) newSL = sl;
              }
           }
        }
         ...
         if(TimeCurrent() - lastLog >= 60)
           {
            ...
            Print("DIAG flat: body=", DoubleToString(body, _Digits),
                  " min=", DoubleToString(Skala(InpCandleMin, InpATRCandleMin), _Digits),
